...
if __name__ == "__main__":
    ...
    parser.add_argument('--lam', action='store', dest='lamMax',
                        help='the range of lambda', default='0.5')
    print('')
    ...
    for lam in range(1, int(results.lamMax) * 10):
        model = get_class(results.model)()
        model.LAMBDA = lam / 10
        model.train(trainSentences)
        print('when lambda is {}'.format(model.LAMBDA))
        ...
        if results.jumble == 'True':
            print('Jumbled sentences: True answer perplexity: = %.5f' % Tester.computePerplexity(model,
                                                                                                 Tester.getCorrectSentences(
                                                                                                     jumbleProblems)))
